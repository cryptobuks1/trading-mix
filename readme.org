...
#+BEGIN_SRC python :var sqlfile=ask-for-datafile
  ...
  data_analysis = setup_analysis(window_generator(3600 * 5,
                                                  600,
                                                  ,**db))
  sim = simulation()
  ...
  def plot_peak(data):
      data_analysis.plot(data['result']['xfit'],
                         data['result']['yfit'],
                         clear=False)
  ...
  def onNewPeak(peak_analysis):
      data_analysis.play_pause_handler()
      data_analysis.plot(peak_analysis['xpeak'],
                         peak_analysis['ypeak'],
                         clear=False)


  bind(TradingEvents.data.fget(data_analysis.events),
       partial(plot_data,
               data_analysis.fig,
               data_analysis.ax))
  bind(TradingEvents.data.fget(data_analysis.events),
       partial(analyseData,
               peakConf,
               foundPeakEvent=TradingEvents.foundPeak.fget(data_analysis.events)))
  bind(TradingEvents.foundPeak.fget(data_analysis.events), plot_peak)
  bind(data_analysis.events.newPeak, onNewPeak)
  sim.trade_on_peak(data_analysis.events)
  bind(data_analysis.events.foundPeak, partial(onPeak,
