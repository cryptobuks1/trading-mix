...
:RESULTS:
[master 250b685] New data added data/ohcl-2018-09-15-18:27:22.sqlite
 1 file changed, 0 insertions(+), 0 deletions(-)
 create mode 100644 data/ohcl-2018-09-15-18:27:22.sqlite
:END:
...
#+BEGIN_SRC python :var sqlfile=ask-for-datafile
  from trading.plot import axis_with_dates_x
  from trading.sql import sqlite_connect, time_range
  from datetime import datetime
  from trading.data import window_generator, window, extract, fit, analyseData
  from trading.octave import conf as peakConf
  from warnings import filterwarnings
  from numpy import RankWarning
  from numpy.linalg import LinAlgError

  filterwarnings('error', "", RankWarning)
  ...
  db = sqlite_connect(sqlfile)
  start, end = time_range(**db)
  latest_order_epoc = {'epoc': start}


  def latest_order_epoc_fn():
      global latest_order_epoc
      print(latest_order_epoc['epoc'])
      return latest_order_epoc['epoc']


  for pos in range(latest_order_epoc_fn(), end, 600):
      data = window(None, start=latest_order_epoc_fn(), end=pos, **db)
      try:
          analysis = analyseData(peakConf,
                                 data,
                                 fit_order=3,
                                 fit_full=False,
                                 fit_cov=True)
          ...
          try:
              ...
              if analysis['xpeak']:
                  print("Peak")
                  print(analysis['xpeak'])
                  latest_order_epoc['epoc'] = analysis['xpeak'][0]
              x = analysis['x']
              xfit = analysis['xfit']
              y = analysis['y']
              yfit = analysis['yfit']
      ...
      xd = [datetime.fromtimestamp(x_) for x_ in x]
      sxd = [datetime.fromtimestamp(x_) for x_ in xfit]
      ax.clear()
      ax.set_xlim(min(xd), max(xd))
      ax.set_ylim(min(y), max(y))
      ax.plot(xd, y)
