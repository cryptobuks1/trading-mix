...
(define (date-range connection start end)
  ...
  (define start-s-curve (+ (* 2 3600) (apply find-seconds start)))
  (define end-s-curve (+ (* 2 3600) (apply find-seconds end)))
...
(module+ test
  ...
  (define dr(date-range *db*
                        '(0 45 20 21 8 2018)
                        '(0 20 23 21 8 2018)))
  (map (lambda (dv)
         (list->vector (list (vector-ref dv 0) 1)))
       dr))
